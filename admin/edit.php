...
if(isset($_POST['savepost'])){
	...
	$relatedapps = addslashes($_POST['appids']);
	$status = $_POST['status'];
	...
	if(!empty($tags)){
        deletekeywordconn($db,'article',$id);
        handlekeywords($db,$tags,'article',$id);
    }
	

	//update 主表
	$sql = "update articles set title='".$title."',remark='".$remark."',tags='".$tags."',status=".$status.",relatedapps='".$relatedapps."',`subtitle`='".$subtitle."' where id=".$id;
	$db->query($sql);
	if(!empty($listimage)){
