...
$aColumns = array('id','title','alias','description','addtime','status','tags');
...
//total num
$totalquery = $db->fetch_first("select count(*) as num from collections where status=1");
...
foreach($datalist as $datatemp){
    ...
    $item[] = '<a class="btn btn-success" target="_blank" href="http://www.topwindata.com/windows10collections/'.$datatemp['alias'].'.html"><i class="glyphicon glyphicon-zoom-in icon-white"></i>
                View
            </a>
            <a class="btn btn-info" href="editCollection.php?id='.$datatemp['id'].'">
